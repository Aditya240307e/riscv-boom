...
/** IO bundle to interact with the ROB
  ...
  *   number of wakeup ports to the rob
  * @param numFpuPorts
  *   number of fpu ports that will write back fflags
  */
class RobIo(
  ...
  val enq_partial_stall = Input(
    Bool()
  ) // we're dispatching only a partial packet,
  ...
  val wb_resps = Flipped(
    Vec(numWakeupPorts, Valid(new ExeUnitResp(xLen max fLen + 1)))
  )
...
/** Bundle to communicate exceptions to CSRFile
  *
  * TODO combine FlushSignals and ExceptionSignals (currently timed to different
  * cycles).
  */
  ...
  def getType(
    ...
    val ret =
      Mux(
        !valid,
        none,
        Mux(i_eret, eret, Mux(i_xcpt, xcpt, Mux(i_refetch, refetch, next)))
      )
    ret
  }
}
...
/** Bundle for debug ROB signals These should not be synthesized!
  */
...
/** Reorder Buffer to keep track of dependencies and inflight instructions
  ...
  *   number of wakeup ports to the ROB
  * @param numFpuPorts
  *   number of FPU units that will write back fflags
  */
class Rob(
    val numWakeupPorts: Int,
  ...
  val rob_head_idx =
    if (coreWidth == 1) rob_head else Cat(rob_head, rob_head_lsb)

  val rob_tail = RegInit(0.U(log2Ceil(numRobRows).W))
  val rob_tail_lsb = RegInit(0.U((1 max log2Ceil(coreWidth)).W))
  val rob_tail_idx =
    if (coreWidth == 1) rob_tail else Cat(rob_tail, rob_tail_lsb)
  ...
  val rob_head_fflags = Wire(
    Vec(
      coreWidth,
      Valid(UInt(freechips.rocketchip.tile.FPConstants.FLAGS_SZ.W))
    )
  )
  ...
  val rob_unsafe_masked = WireInit(
    VecInit(Seq.fill(numRobRows << log2Ceil(coreWidth)) { false.B })
  )
  ...
  if (usingTrace) {
    ...
    rob_debug_inst_mem.write(
      rob_tail,
      rob_debug_inst_wdata,
      rob_debug_inst_wmask
    )
    rob_debug_inst_rdata := rob_debug_inst_mem.read(
      rob_head,
  ...
  val brupdate_b2_rob_clr_oh =
    IsYoungerMask(brupdate_b2_rob_row, rob_head, numRobRows)
  val brupdate_b2_rob_bank_idx = GetBankIdx(io.brupdate.b2.uop.rob_idx)
  val brupdate_b2_rob_bank_clr_oh = ~MaskLower(
    UIntToOH(brupdate_b2_rob_bank_idx)
  )
  ...
  val compactUopWidth =
    1 + log2Ceil(ftqSz) + 1 + 1 + 2 + lregSz + maxPregSz + maxPregSz
  ...
  val rob_compact_uop_mem =
    SyncReadMem(numRobRows, Vec(coreWidth, UInt(compactUopWidth.W)))
  val rob_compact_uop_wdata = VecInit(
    io.enq_uops.map(u => uop_to_compact(u).asUInt)
  )
  ...
  val rob_compact_uop_bypassed = (0 until coreWidth) map { w =>
    Mux(
      rob_head === RegNext(rob_tail) && RegNext(io.enq_valids(w)),
      RegNext(rob_compact_uop_wdata(w)),
      Mux(
        rob_head === ShiftRegister(rob_tail, 2) && ShiftRegister(
          io.enq_valids(w),
          2
        ),
  ...
  val rob_fflags = Seq.fill(coreWidth)(
  ...
  for (w <- 0 until coreWidth) {
    ...
    val rob_predicated = Reg(
      Vec(numRobRows, Bool())
    ) // Was this instruction predicated out?
    val rob_fflags = Reg(
      Vec(
        numRobRows,
        Valid(Bits(freechips.rocketchip.tile.FPConstants.FLAGS_SZ.W))
      )
    )
    ...
    when(
      io.sidecar_wb.valid && MatchBank(GetBankIdx(io.sidecar_wb.bits.rob_idx))
    ) {
      ...
      rob_unsafe(row_idx) := false.B
      assert(
        rob_val(row_idx) === true.B,
        "[rob] Sidecar writeback to an invalid ROB entry"
      )
    }
    ...
    for (clr_rob_idx <- io.lsu_clr_bsy) {
      when(clr_rob_idx.valid && MatchBank(GetBankIdx(clr_rob_idx.bits))) {
        ...
        assert(
          rob_val(cidx) === true.B,
          "[rob] store writing back to invalid entry."
        )
        assert(
          rob_bsy(cidx) === true.B,
          "[rob] store writing back to a not-busy entry."
        )
      }
    }
    ...
    when(
      io.csr_replay.valid && MatchBank(
        GetBankIdx(io.csr_replay.bits.uop.rob_idx)
      )
    ) {
    ...
    // The following is the original line in the original source code:
    ...
    io.commit.uops(w) := compact_to_uop(
      rob_compact_uop_bypassed(w),
      rob_uop(rob_head)
    )
    ...
    when(
      ...
      rob_uop(
        GetRowIdx(io.brupdate.b2.uop.rob_idx)
      ).taken := io.brupdate.b2.taken
    }
    ...
    for (i <- 0 until numRobRows) {
      rob_unsafe_masked((i << log2Ceil(coreWidth)) + w) := rob_val(
        i
      ) && (rob_unsafe(i) || rob_exception(i))
    }
    // Read unsafe status of PNR row.
    rob_pnr_unsafe(w) := rob_val(rob_pnr) && (rob_unsafe(
      rob_pnr
    ...
    for (i <- 0 until numWakeupPorts) {
      ...
      assert(
        !(io.wb_resps(i).valid && MatchBank(GetBankIdx(rob_idx)) &&
          temp_uop.dst_rtype =/= RT_X && temp_uop.pdst =/= io
            .wb_resps(i)
            .bits
            .uop
            .pdst),
  ...
  var block_commit =
    (rob_state =/= s_normal) && (rob_state =/= s_wait_till_empty) || RegNext(
      exception_thrown
  ...
  for (w <- 0 until coreWidth) {
    will_throw_exception = (can_throw_exception(
      w
    ) && !block_commit && !block_xcpt) || will_throw_exception
  ...
  val is_mini_exception = io.com_xcpt.bits.cause
    .isOneOf(MINI_EXCEPTION_MEM_ORDERING, MINI_EXCEPTION_CSR_REPLAY)
  ...
  val insn_sys_pc2epc =
    rob_head_vals.reduce(_ | _) && PriorityMux(
      rob_head_vals,
      io.commit.uops.map { u => u.is_sys_pc2epc }
    )
  ...
  assert(
    !(PopCount(flush_commit_mask) > 1.U),
  ...
  val flush_uop = Mux(
    exception_thrown,
    com_xcpt_uop,
    Mux1H(flush_commit_mask, io.commit.uops)
  )
  ...
  val fflags = Wire(
    Vec(coreWidth, UInt(freechips.rocketchip.tile.FPConstants.FLAGS_SZ.W))
  )
  ...
  when(!(io.flush.valid || exception_thrown)) {
    ...
    val lxcpt_older = !io.csr_replay.valid || (IsOlder(
      io.lxcpt.bits.uop.rob_idx,
      io.csr_replay.bits.uop.rob_idx,
      rob_head_idx
    ) && io.lxcpt.valid)
    ...
    when(new_xcpt_valid) {
      ...
      r_xcpt_badvaddr := AlignPCToBoundary(io.xcpt_fetch_pc, icBlockBytes) | io
        .enq_uops(idx)
        .pc_lob
  ...
  io.com_load_is_at_rob_head := RegNext(
    rob_head_uses_ldq(PriorityEncoder(rob_head_vals.asUInt)) &&
      !will_commit.reduce(_ || _)
  )
