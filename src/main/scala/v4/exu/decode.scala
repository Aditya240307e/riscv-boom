...
// scalastyle:off
/** Abstract trait giving defaults and other relevant values to different Decode
  * constants/
  */
object DecodeTables
  ...
  def X32_table: Seq[(BitPat, List[BitPat])] = {
...
//   dontTouch(cs_legal)
  ...
  require(fLen >= 64)
  val illegal_rm = inst(14, 12)
    .isOneOf(5.U, 6.U) || (inst(14, 12) === 7.U && io.fcsr_rm >= 5.U)
...
//     cs.div && !csr.io.status.isa('m'-'a') || TODO check for illegal div instructions
  ...
  val (xcpt_valid, xcpt_cause) = checkExceptions(
    List(
      (
        io.interrupt && !io.enq.uop.is_sfb,
        io.interrupt_cause
      ), // Disallow interrupts while we are handling a SFB
  ...
  uop.iq_type(IQ_UNQ) := Seq(FC_MUL, FC_DIV, FC_CSR, FC_I2F)
    .map { c => cs.fu_code(c) }
    .reduce(_ || _)
  uop.iq_type(IQ_ALU) := Seq(FC_ALU).map { c => cs.fu_code(c) }.reduce(_ || _)
  uop.iq_type(IQ_MEM) := Seq(FC_AGEN, FC_DGEN)
    .map { c => cs.fu_code(c) }
    .reduce(_ || _)
  uop.iq_type(IQ_FP) := Seq(FC_FPU, FC_FDV, FC_F2I)
    .map { c => cs.fu_code(c) }
    .reduce(_ || _)
  ...
  uop.lrs1_rtype := Mux(
    cs.rs1_type === RT_FIX && LRS1 === 0.U,
    RT_ZERO,
    cs.rs1_type
  )
  uop.lrs2_rtype := Mux(
    cs.rs2_type === RT_FIX && LRS2 === 0.U,
    RT_ZERO,
    cs.rs2_type
  )
  ...
  uop.mem_size := Mux(
    cs.mem_cmd.isOneOf(M_SFENCE, M_FLUSH_ALL),
    Cat(LRS2 =/= 0.U, LRS1 =/= 0.U),
    inst(13, 12)
  )
  ...
  uop.is_rocc := inst(6, 0).isOneOf(
    "b0001011".U,
    "b0101011".U,
    "b1111011".U
  ) && inst(14, 12).isOneOf(0.U, 2.U, 3.U, 4.U, 6.U, 7.U)
  ...
  val di24_20 =
    Mux(cs.imm_sel === IS_B || cs.imm_sel === IS_S, inst(11, 7), inst(24, 20))
  ...
  uop.op1_sel := OP1_RS1
  when(
    inst === LUI || inst === CSRRWI || inst === CSRRSI || inst === CSRRCI ||
  ...
  when(cs.is_amo || inst === CSRRW || inst === CSRRS || inst === CSRRC) {
    ...
    inst === CSRRWI || inst === CSRRSI || inst === CSRRCI ||
    ...
    cs.imm_sel === IS_U || cs.imm_sel === IS_I || cs.imm_sel === IS_S
  ) {
    uop.op2_sel := OP2_IMM
  }
...
/** Smaller Decode unit for the Frontend to decode different branches. Accepts
  * EXPANDED RVC instructions
  */
...
////                               |  |  |  |  |
  ...
  io.out.is_ret := cs_is_jalr && GetRs1(io.inst) === BitPat("b00?01") && GetRd(
    io.inst
  ) === X0

  io.out.target := Mux(
    cs_is_br,
    ComputeBranchTarget(io.pc, io.inst, xLen),
  ...
  io.out.sfb_offset.valid := cs_is_br && !io.inst(
    31
  ) && br_offset =/= 0.U && (br_offset >> log2Ceil(icBlockBytes)) === 0.U
...
/** Track the current "branch mask", and give out the branch mask to each
  * micro-op in Decode (each micro-op in the machine has a branch mask which
  * says which branches it is being speculated under).
  *
  * @param pl_width
  *   pipeline width for the processor
  */
class BranchMaskGenerationLogic(val pl_width: Int)(implicit p: Parameters)
    extends BoomModule {
  val io = IO(new Bundle {
  ...
  for (w <- 0 until pl_width) {
    ...
    allocate_mask =
      Mux(io.is_branch(w), tag_masks(w) | allocate_mask, allocate_mask)
  }
