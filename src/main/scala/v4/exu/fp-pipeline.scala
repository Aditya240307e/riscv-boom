...
/** Top level datapath that wraps the floating point issue window, regfile, and
  * arithmetic units.
  */
class FpPipeline(implicit p: Parameters)
    extends BoomModule
  ...
  val numWakeupPorts = fpIssueParams.issueWidth + numLlPorts + 1
  ...
  val io = IO(new Bundle {
    ...
    val fcsr_rm =
      Input(UInt(width = freechips.rocketchip.tile.FPConstants.RM_SZ.W))
    val status = Input(new freechips.rocketchip.rocket.MStatus())

    val dis_uops = Vec(dispatchWidth, Flipped(Decoupled(new MicroOp)))

    // +1 for recoding.
    val ll_wports = Flipped(
      Vec(lsuWidth, Valid(new ExeUnitResp(fLen + 1)))
    ) // from memory unit
    val from_int =
      Flipped(Decoupled(new ExeUnitResp(fLen + 1))) // from integer RF
  ...
  // Wakeup
// Manual mapping to bypass the strict Vector length check
  for (i <- 0 until (issue_unit.io.wakeup_ports.length min fp_wakeups.length)) {
    issue_unit.io.wakeup_ports(i) := fp_wakeups(i)
  }
  ...
  ll_wbarb.io.in(0).valid := RegNext(
    io.ll_wports(0).valid &&
      !IsKilledByBranch(io.brupdate, io.flush_pipeline, io.ll_wports(0).bits)
  )
  ll_wbarb.io.in(0).bits := RegNext(
    UpdateBrMask(io.brupdate, io.ll_wports(0).bits)
  )
  ll_wbarb.io.in(0).bits.data := recode(
    RegNext(io.ll_wports(0).bits.data),
  ...
  fregfile.io
    .write_ports(0)
    .valid := ll_wbarb.io.out.valid && ll_wbarb.io.out.bits.uop.dst_rtype === RT_FLT
  ...
  for (i <- 1 until lsuWidth) {
    ...
    fregfile.io.write_ports(w_cnt).bits.addr := RegNext(
      io.ll_wports(i).bits.uop.pdst
    )
    fregfile.io.write_ports(w_cnt).bits.data := recode(
  ...
  for (eu <- exe_units) {
    fregfile.io
      .write_ports(w_cnt)
      .valid := eu.io_fpu_resp.valid && eu.io_fpu_resp.bits.uop.dst_rtype === RT_FLT
  ...
  for (w <- 0 until fpWidth) {
    fp_bypasses(w).valid := exe_units(w).io_fpu_resp.valid && exe_units(
      w
    ).io_fpu_resp.bits.uop.dst_rtype === RT_FLT
  ...
  for (i <- 1 until lsuWidth) {
    val wb = RegNext(
      UpdateBrMask(io.brupdate, io.flush_pipeline, io.ll_wports(i))
    )
  ...
  override def toString: String =
    (BoomCoreStringPrefix("===FP Pipeline===") + "\n"
      ...
      + fregfile.toString
      + BoomCoreStringPrefix("Num Wakeup Ports      : " + numWakeupPorts))
}
