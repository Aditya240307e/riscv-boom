...
class Wakeup(implicit p: Parameters) extends BoomBundle with HasBoomUOP {
  val bypassable = Bool()
...
class MemGen(implicit p: Parameters) extends BoomBundle with HasBoomUOP {
  val data = UInt(xLen.W)
}

class CSRResp(implicit p: Parameters) extends BoomBundle with HasBoomUOP {
  val data = UInt(xLen.W)
  val addr = UInt(CSR.ADDRSZ.W)
}

abstract class ExecutionUnit(name: String)(implicit p: Parameters)
    extends BoomMultiIOModule {
  ...
  def get_ready_fu_types(): Vec[Bool] = {
    val r = WireInit(VecInit(Seq.fill(FC_SZ) { false.B }))
    fu_types.map {
      case (code, ready, _) => { when(ready) { r(code) := true.B } }
    }
    r
  }
  ...
  override def toString = {
    BoomCoreStringPrefix(s"===${name}ExeUnit") +
      fu_types
        .map { case (_, _, s) => BoomCoreStringPrefix(s" - ${s}") }
        .reduce(_ + _)
  }
  ...
  arb_uop.valid := io_iss_uop.valid && !IsKilledByBranch(
    io_brupdate,
    io_kill,
    io_iss_uop.bits
  )
  ...
  rrd_uop.valid := arb_uop.valid && !IsKilledByBranch(
    io_brupdate,
    io_kill,
    arb_uop.bits
  )
  ...
  exe_uop.valid := rrd_uop.valid && !IsKilledByBranch(
    io_brupdate,
    io_kill,
    rrd_uop.bits
  )
...
trait HasIrfReadPorts { this: ExecutionUnit =>
  ...
  val io_rrd_irf_bypasses = IO(
    Input(Vec(coreWidth + lsuWidth, Valid(new ExeUnitResp(xLen))))
  )

  def rrd_bypass_hit(prs: UInt, rdata: UInt): (Bool, UInt) = {
    val hits = io_rrd_irf_bypasses map { b =>
      b.valid && prs === b.bits.uop.pdst
    }
    (
      hits.reduce(_ || _),
      Mux(
        hits.reduce(_ || _),
        Mux1H(hits, io_rrd_irf_bypasses.map(_.bits.data)),
        rdata
      )
    )
  }
  ...
  def rebusied(prs: UInt): Bool = {
    io_arb_rebusys
      .map { r => r.valid && r.bits.rebusy && r.bits.uop.pdst === prs }
      .reduce(_ || _)
  }

  io_arb_irf_reqs(
    0
  ).valid := arb_uop.valid && arb_uop.bits.lrs1_rtype === RT_FIX && !arb_uop.bits.iw_p1_bypass_hint
  io_arb_irf_reqs(0).bits := arb_uop.bits.prs1

  if (nReaders == 2) {
    io_arb_irf_reqs(
      1
    ).valid := arb_uop.valid && arb_uop.bits.lrs2_rtype === RT_FIX && !arb_uop.bits.iw_p2_bypass_hint
    io_arb_irf_reqs(1).bits := arb_uop.bits.prs2
  }
  val arb_rebusied_prs1 =
    arb_uop.bits.lrs1_rtype === RT_FIX && rebusied(arb_uop.bits.prs1)
  val arb_rebusied_prs2 = arb_uop.bits.lrs2_rtype === RT_FIX && rebusied(
    arb_uop.bits.prs2
  ) && (nReaders == 2).B
  ...
  val (rs1_hit, rs1_data) =
    rrd_bypass_hit(rrd_uop.bits.prs1, io_rrd_irf_resps(0))
  assert(
    !(rrd_uop.valid && rrd_uop.bits.lrs1_rtype === RT_FIX && rrd_uop.bits.iw_p1_bypass_hint && !rs1_hit)
  )
  exe_rs1_data := Mux(rrd_uop.bits.lrs1_rtype === RT_ZERO, 0.U, rs1_data)

  if (nReaders == 2) {
    val (rs2_hit, rs2_data) =
      rrd_bypass_hit(rrd_uop.bits.prs2, io_rrd_irf_resps(1))
    assert(
      !(rrd_uop.valid && rrd_uop.bits.lrs2_rtype === RT_FIX && rrd_uop.bits.iw_p2_bypass_hint && !rs2_hit)
    )
...
trait HasImmrfReadPort { this: ExecutionUnit =>
  ...
  val exe_imm_data = RegNext(
    Mux(
      rrd_uop.bits.imm_sel === IS_SH,
...
trait HasFrfReadPorts { this: ExecutionUnit =>
  ...
  val io_rrd_frf_bypasses = IO(
    Input(Vec(fpWidth, Valid(new ExeUnitResp(fLen + 1))))
  )

  def rrd_bypass_hit(prs: UInt, rdata: UInt): (Bool, UInt) = {
    val hits = io_rrd_frf_bypasses map { b =>
      b.valid && prs === b.bits.uop.pdst
    }
    (
      hits.reduce(_ || _),
      Mux(
        hits.reduce(_ || _),
        Mux1H(hits, io_rrd_frf_bypasses.map(_.bits.data)),
        rdata
      )
    )
  }
  ...
  io_arb_frf_reqs(
    0
  ).valid := arb_uop.valid && arb_uop.bits.lrs1_rtype === RT_FLT && !arb_uop.bits.iw_p1_bypass_hint
  io_arb_frf_reqs(0).bits := arb_uop.bits.prs1
  io_arb_frf_reqs(
    1
  ).valid := arb_uop.valid && arb_uop.bits.lrs2_rtype === RT_FLT && !arb_uop.bits.iw_p2_bypass_hint
  io_arb_frf_reqs(1).bits := arb_uop.bits.prs2
  io_arb_frf_reqs(
    2
  ).valid := arb_uop.valid && arb_uop.bits.frs3_en && !arb_uop.bits.iw_p3_bypass_hint
  ...
  val (rs1_hit, rs1_data) =
    rrd_bypass_hit(rrd_uop.bits.prs1, io_rrd_frf_resps(0))
  assert(
    !(rrd_uop.valid && rrd_uop.bits.lrs1_rtype === RT_FLT && rrd_uop.bits.iw_p1_bypass_hint && !rs1_hit)
  )
  exe_rs1_data := rs1_data

  val (rs2_hit, rs2_data) =
    rrd_bypass_hit(rrd_uop.bits.prs2, io_rrd_frf_resps(1))
  assert(
    !(rrd_uop.valid && rrd_uop.bits.lrs2_rtype === RT_FLT && rrd_uop.bits.iw_p2_bypass_hint && !rs2_hit)
  )
  exe_rs2_data := rs2_data

  val (rs3_hit, rs3_data) =
    rrd_bypass_hit(rrd_uop.bits.prs3, io_rrd_frf_resps(2))
...
trait HasFtqReadPort { this: ExecutionUnit =>
  ...
  io_arb_ftq_reqs(
    0
  ).valid := arb_uop.valid && (arb_uop.bits.op1_sel === OP1_PC || arb_uop.bits.is_sfb_br)
  io_arb_ftq_reqs(0).bits := arb_uop.bits.ftq_idx

  // Only JALR checks the next-pc
  io_arb_ftq_reqs(
    1
  ).valid := arb_uop.valid && (arb_uop.bits.br_type === B_JR || arb_uop.bits.is_sfb_br)
...
class MemExeUnit(
  ...
  when(io_squash_iss || arb_rebusied) {
    val will_replay = arb_uop.valid && !IsKilledByBranch(
      io_brupdate,
      io_kill,
      arb_uop.bits
    ) && !arb_rebusied
  ...
  val io_agen = if (hasAGen) {
    val loads_saturating =
      exe_uop.valid && exe_uop.bits.uses_ldq && exe_uop.bits.fu_code(FC_AGEN)
...
class UniqueExeUnit(
  ...
  val (io_ifpu_resp) = if (hasIfpu) {
    ...
    val queue = Module(
      new BranchKillableQueue(
        new ExeUnitResp(xLen + 1),
        entries = intToFpLatency + 6
      )
    ) // TODO being overly conservative
...
class FPExeUnit(val hasFDiv: Boolean = false, val hasFpiu: Boolean = false)(
    implicit p: Parameters
  ...
  when(io_squash_iss) {
    val will_replay =
      arb_uop.valid && !IsKilledByBranch(io_brupdate, io_kill, arb_uop.bits)
    arb_uop.valid := will_replay
    arb_uop.bits := UpdateBrMask(io_brupdate, arb_uop.bits)
    arb_uop.bits.iw_p1_bypass_hint := false.B
    arb_uop.bits.iw_p2_bypass_hint := false.B
  ...
  fpu.io.req.valid := exe_uop.valid && (
    exe_uop.bits.fu_code(FC_FPU) || (if (hasFpiu) exe_uop.bits.fu_code(FC_F2I)
                                     else false.B)
  )
  ...
  io_fpu_resp.valid := fpu.io.resp.valid && !fpu.io.resp.bits.uop
    .fu_code(FC_F2I)
  ...
  val (io_fpiu_resp, io_dgen) = if (hasFpiu) {
    ...
    dgen.valid := RegNext(
      exe_uop.valid && exe_uop.bits.uses_stq && !IsKilledByBranch(
        io_brupdate,
        io_kill,
        exe_uop.bits
      )
