...
class IssueSlotIO(val numWakeupPorts: Int)(implicit p: Parameters)
    extends BoomBundle {
  val valid = Output(Bool())
  val will_be_valid = Output(
    Bool()
  ) // TODO code review, do we need this signal so explicitely?
  ...
  val clear = Input(
    Bool()
  ) // entry being moved elsewhere (not mutually exclusive with grant)
...
class IssueSlot(val numWakeupPorts: Int, val isMem: Boolean, val isFp: Boolean)(
  ...
  val veto_signal =
    if (numWakeupPorts > 3) io.wakeup_ports(3).valid else false.B
  val prs1_matches = data_wakeup_ports.map { w =>
    w.bits.uop.pdst === slot_uop.prs1
  }
  val prs2_matches = data_wakeup_ports.map { w =>
    w.bits.uop.pdst === slot_uop.prs2
  }
  val prs3_matches = data_wakeup_ports.map { w =>
    w.bits.uop.pdst === slot_uop.prs3
  }

  val prs1_wakeups = (data_wakeup_ports zip prs1_matches).map { case (w, m) =>
    w.valid && m
  }
  val prs2_wakeups = (data_wakeup_ports zip prs2_matches).map { case (w, m) =>
    w.valid && m
  }
  val prs3_wakeups = (data_wakeup_ports zip prs3_matches).map { case (w, m) =>
    w.valid && m
  }

  val prs1_rebusys = (data_wakeup_ports zip prs1_matches).map { case (w, m) =>
    w.bits.rebusy && m
  }
  val prs2_rebusys = (data_wakeup_ports zip prs2_matches).map { case (w, m) =>
    w.bits.rebusy && m
  }

  val bypassables = data_wakeup_ports.map { w => w.bits.bypassable }
  val speculative_masks = data_wakeup_ports.map { w => w.bits.speculative_mask }
  ...
  when(
    (prs1_rebusys.reduce(
      _ || _
    ) || ((io.child_rebusys & slot_uop.iw_p1_speculative_child) =/= 0.U)) &&
  ...
  when(
    (prs2_rebusys.reduce(
      _ || _
    ) || ((io.child_rebusys & slot_uop.iw_p2_speculative_child) =/= 0.U)) &&
  ...
  when(
    io.pred_wakeup_port.valid && io.pred_wakeup_port.bits === slot_uop.ppred
  ) {
