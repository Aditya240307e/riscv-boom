...
class IssueUnitAgeMatrix(
  ...
  for (w <- 0 until dispatchWidth) {
    ...
    val prs1_wakeups = (data_wakeup_ports zip prs1_matches).map {
      case (wu, m) => wu.valid && m
    }
    val prs2_wakeups = (data_wakeup_ports zip prs2_matches).map {
      case (wu, m) => wu.valid && m
    }
    val prs3_wakeups = (data_wakeup_ports zip prs3_matches).map {
      case (wu, m) => wu.valid && m
    }

    val prs1_rebusys = (data_wakeup_ports zip prs1_matches).map {
      case (wu, m) => wu.bits.rebusy && m
    }
    val prs2_rebusys = (data_wakeup_ports zip prs2_matches).map {
      case (wu, m) => wu.bits.rebusy && m
    }

    val bypassables = data_wakeup_ports.map { wu => wu.bits.bypassable }
    val speculative_masks = data_wakeup_ports.map { wu =>
      wu.bits.speculative_mask
    }

    when(prs1_wakeups.reduce(_ || _)) {
      dis_uops(w).prs1_busy := false.B
      dis_uops(w).iw_p1_speculative_child := Mux1H(
        prs1_wakeups,
        speculative_masks
      )
    ...
    when(
      prs1_rebusys.reduce(_ || _) || ((io.child_rebusys & io
        .dis_uops(w)
        .bits
    ...
    when(prs2_wakeups.reduce(_ || _)) {
      ...
      dis_uops(w).iw_p2_speculative_child := Mux1H(
        prs2_wakeups,
        speculative_masks
      )
    ...
    when(
      prs2_rebusys.reduce(_ || _) || ((io.child_rebusys & io
        .dis_uops(w)
        .bits
    ...
    when(
      io.pred_wakeup_port.valid && io.pred_wakeup_port.bits === io
        .dis_uops(w)
        .bits
        .ppred
    ) {
    ...
    if (iqType == IQ_UNQ) {
      when(io.dis_uops(w).bits.fu_code(FC_I2F)) {
        dis_uops(w).prs2 := Cat(
          io.dis_uops(w).bits.fp_rm,
          io.dis_uops(w).bits.fp_typ
        )
      }
    ...
    if (iqType == IQ_MEM) {
      ...
      when(
        io.dis_uops(w)
          .bits
          .uses_stq && io.dis_uops(w).bits.lrs2_rtype === RT_FLT
      ) {
  ...
  val slots = (0 until numIssueSlots) map { w =>
    Module(new IssueSlot(numWakeupPorts, iqType == IQ_MEM, iqType == IQ_FP))
  }
  val issue_slots = VecInit(slots.map(_.io))

  for (i <- 0 until numIssueSlots) {
    val n = issue_slots(i).numWakeupPorts
    issue_slots(i).wakeup_ports := io.wakeup_ports.take(n)
  ...
  assert(
    PopCount(issue_slots.map(s => s.grant)) <= issueWidth.U,
    "[issue] window giving out too many grants."
  )
  ...
  for (i <- 0 until numIssueSlots) {
    ...
    dis_scan = Mux(
      dis_scan < dispatchWidth.U && slots_empty(i),
      dis_scan + 1.U,
      dis_scan
    )
  }
  ...
  val slots_age_matrix = RegInit(
    VecInit.tabulate(numIssueSlots, numIssueSlots) { (_, _) => false.B }
  )
  ...
  val slots_valid_alloc = WireDefault(
    VecInit(slots.map(s => !s.io.valid & s.io.will_be_valid))
  )
  val slots_valid_dealloc = WireDefault(
    VecInit(slots.map(s => s.io.valid & !s.io.will_be_valid))
  )
  ...
  for (i <- 0 until issueWidth) {
    for (j <- 0 until numIssueSlots) {
      ...
      val fu_code_match = (issue_slots(j).iss_uop.fu_code zip io.fu_types(i))
        .map { case (r, c) =>
          r && c
        }
        .reduce(_ || _)
  ...
  val iss_signals = Wire(
    Vec(issueWidth, Valid(UInt(log2Ceil(numIssueSlots).W)))
  )
  ...
  if (!params.useFullIssueSel) {
    ...
    for (i <- 0 until issueWidth) {
      for (j <- 0 until numIssueSlots) {
        ...
        issue_oldest_oh(i)(j) := !((slots_age_matrix(i).asUInt & iss_ready(
          i
        ).asUInt).orR) && iss_ready(i)(j)
      }
...
// minstret = 186031
    for (i <- 0 until issueWidth) {
      val (old_iss_valid, old_iss_index) =
        ParallelFindOne(issue_oldest_oh(i).asUInt, i % 2 == 0)
      ...
      val (val_iss_valid, val_iss_index) =
        ParallelFindOne(compute_valid_mask.asUInt, i % 2 == 1)
      iss_signals(i).valid := oldest_iss(i).valid | val_iss_valid
      iss_signals(i).bits := Mux(
        oldest_iss(i).valid,
        oldest_iss(i).bits,
        val_iss_index
      )
      for (j <- 0 until i) {
        assert(
          !(iss_signals(i).valid & iss_signals(j).valid) | (iss_signals(
            i
          ).bits =/= iss_signals(j).bits)
    ...
    for (i <- 0 until issueWidth) {
      ...
      when(issued_index(i).valid) {
        for (j <- (i + 1) until issueWidth) {
          iss_ready(j)(
            issued_index(i).bits
          ) := false.B // don't issue this entry in the future
        }
      }
    }
