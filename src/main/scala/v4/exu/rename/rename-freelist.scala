...
abstract class AbstractRenameFreeList(
  ...
  val io = IO(new Bundle {
    ...
    // SSI (Veto) signals
    val latch_shadow = Input(Bool()) // Triggered by CSR 0x800
...
class RenameFreeList(
  ...
  val shadow_free_list = RegInit(UInt(numPregs.W), io.initial_allocation)

  when(io.latch_shadow) {
    shadow_free_list := free_list
  }
  ...
  val alloc_masks = (allocs zip io.reqs).scanRight(0.U(n.W)) {
    case ((a, r), m) => m | a & Fill(n, r)
  }

  val sel_mask = (sels zip sel_fire) map { case (s, f) =>
    s & Fill(n, f)
  } reduce (_ | _)

  val br_deallocs = br_alloc_lists(io.brupdate.b2.uop.br_tag) & Fill(
    n,
    io.brupdate.b2.mispredict
  )
  val com_deallocs = RegNext(io.dealloc)
    .map(d => UIntToOH(d.bits)(numPregs - 1, 0) & Fill(n, d.valid))
    .reduce(_ | _)
  val rollback_deallocs = spec_alloc_list & Fill(n, io.rollback)
  val dealloc_mask = com_deallocs | br_deallocs | rollback_deallocs

  val com_despec = io.despec
    .map(d => UIntToOH(d.bits)(numPregs - 1, 0) & Fill(n, d.valid))
    .reduce(_ | _)

  when(io.veto_restore) {
    free_list := shadow_free_list
    spec_alloc_list := 0.U
  }.otherwise {
    free_list := (free_list & ~sel_mask) | dealloc_mask
    spec_alloc_list := (spec_alloc_list | alloc_masks(
      0
    )) & ~dealloc_mask & ~com_despec
  }
  ...
  if (enableSuperscalarSnapshots) {
    ...
    for (i <- 0 until maxBrCount) {
      val list_req =
        VecInit(io.ren_br_tags.map(tag => tag.bits === i.U)).asUInt & br_slots
    ...
    val br_snapshot_tag =
      Mux1H(io.ren_br_tags.map(_.valid), io.ren_br_tags.map(_.bits))
  ...
  for (w <- 0 until allocWidth) {
    ...
    r_valid := (r_valid && !io.reqs(w) || can_sel) && !io.veto_restore
    sel_fire(w) := (!r_valid || io.reqs(w)) && can_sel && !io.veto_restore

    io.alloc_pregs(w).bits := r_sel
    io.alloc_pregs(w).valid := r_valid
  }

  io.debug_freelist := free_list | io.alloc_pregs
    .map(p => UIntToOH(p.bits)(n - 1, 0) & Fill(n, p.valid))
    .reduce(_ | _)

  if (!isImm)
    assert(
      !(io.debug_freelist & dealloc_mask).orR,
      "[freelist] Returning a free physical register."
    )
}
...
class BankedRenameFreeList(
  ...
  val freelists = (0 until plWidth).map { w =>
    val freelist =
      Module(new RenameFreeList(1, plWidth, numPregs / plWidth, false))

    freelist.io.latch_shadow := io.latch_shadow
    freelist.io.veto_restore := io.veto_restore

    val initial = io.initial_allocation.asBools.zipWithIndex
      .filter(_._2 % plWidth == w)
      .map(_._1)
    freelist.io.initial_allocation := VecInit(initial).asUInt
    freelist.io.reqs(0) := false.B

    for (i <- 0 until plWidth) {
      freelist.io.despec(i).valid := io.despec(i).valid && bankIdx(
        io.despec(i).bits
      ) === w.U
      freelist.io.despec(i).bits := io.despec(i).bits >> log2Ceil(plWidth)

      freelist.io.dealloc(i).valid := io.dealloc(i).valid && bankIdx(
        io.dealloc(i).bits
      ) === w.U
  ...
  for (w <- 0 until plWidth) {
    ...
    for (i <- 0 until plWidth) {
      when(sel((i + w) % plWidth)) {
        ...
        io.alloc_pregs(w).bits := Cat(
          freelists(i).io.alloc_pregs(0).bits,
          i.U(log2Ceil(plWidth).W)
        )

        freelists(i).io.ren_br_tags(0).valid := io.ren_br_tags
          .map(_.valid)
          .reduce(_ || _) && !older_than_branch
        freelists(i).io.ren_br_tags(0).bits := Mux1H(
          io.ren_br_tags.map(_.valid),
          io.ren_br_tags.map(_.bits)
        )

        freelists(i).io.ren_br_tags(1).valid := io.ren_br_tags
          .map(_.valid)
          .reduce(_ || _) && older_than_branch
        freelists(i).io.ren_br_tags(1).bits := Mux1H(
          io.ren_br_tags.map(_.valid),
          io.ren_br_tags.map(_.bits)
        )
      }
