...
abstract class AbstractRenameStage(plWidth: Int, numWbPorts: Int)(implicit
    p: Parameters
) extends BoomModule {
  ...
  val ren2_alloc_fire = (ren2_fire zip ren2_alloc_reqs).map({ case (f, r) =>
    f && r
  })
  ...
  for (w <- 0 until plWidth) {
    ...
    r_uop := GetNewUopAndBrMask(
      BypassAllocations(next_uop, ren2_uops, ren2_alloc_fire),
      io.brupdate
    )
...
/** Rename stage that connets the map table, free list, and busy table. Can be
  * used in both the FP pipeline and the normal execute pipeline.
  *
  * @param plWidth
  *   pipeline width
  * @param numWbPorts
  *   number of int writeback ports
  * @param numWbPorts
  *   number of FP writeback ports
  */
class RenameStage(
    plWidth: Int,
    numPhysRegs: Int,
    ...
    extends AbstractRenameStage(plWidth, numWbPorts)(p) {
  val int = !float
  ...
  override def BypassAllocations(
      uop: MicroOp,
      older_uops: Seq[MicroOp],
      alloc_reqs: Seq[Bool]
  ): MicroOp = {
    if (older_uops.size == 0) {
      ...
      val bypass_hits_rs1 = (older_uops zip alloc_reqs) map { case (r, a) =>
        a && r.ldst === uop.lrs1
      }
      val bypass_hits_rs2 = (older_uops zip alloc_reqs) map { case (r, a) =>
        a && r.ldst === uop.lrs2
      }
      val bypass_hits_rs3 = (older_uops zip alloc_reqs) map { case (r, a) =>
        a && r.ldst === uop.lrs3
      }
      val bypass_hits_dst = (older_uops zip alloc_reqs) map { case (r, a) =>
        a && r.ldst === uop.ldst
      }
      ...
      when(do_bypass_rs1) {
        bypassed_uop.prs1 := Mux1H(bypass_sel_rs1, bypass_pdsts)
      }
      when(do_bypass_rs2) {
        bypassed_uop.prs2 := Mux1H(bypass_sel_rs2, bypass_pdsts)
      }
      when(do_bypass_rs3) {
        bypassed_uop.prs3 := Mux1H(bypass_sel_rs3, bypass_pdsts)
      }
      when(do_bypass_dst) {
        bypassed_uop.stale_pdst := Mux1H(bypass_sel_dst, bypass_pdsts)
      }
  ...
  val maptable = Module(
    new RenameMapTable(plWidth, 32, numPhysRegs, false, float)
  )
  val freelist = Module(
    if (
  ...
  val busytable = Module(
    new RenameBusyTable(plWidth, numPhysRegs, numWbPorts, float)
  )
  ...
  for (
    (((ren1, ren2), com), w) <-
      (ren1_uops zip ren2_uops zip io.com_uops).zipWithIndex
  ) {
  ...
  for ((uop, w) <- ren2_uops.zipWithIndex) {
    ...
    uop.pdst := preg
    uop.dis_col_sel := (if (coreWidth > 1)
                          UIntToOH(preg(log2Ceil(coreWidth) - 1, 0))
                        else 0.U(1.W))
  }
  assert(
    !RegNext(io.rollback) || PopCount(
      freelist.io.debug_freelist
    ) === (numPhysRegs - 32).U,
  ...
  assert(
    !(io.wakeups
      .map(x => x.valid && x.bits.uop.dst_rtype =/= rtype)
      .reduce(_ || _)),
  ...
  for ((uop, w) <- ren2_uops.zipWithIndex) {
    ...
    assert(
      !(valid && busy.prs1_busy && rtype === RT_FIX && uop.lrs1_rtype === RT_FIX && uop.lrs1 === 0.U),
      "[rename] x0 is busy??"
    )
    assert(
      !(valid && busy.prs2_busy && rtype === RT_FIX && uop.lrs2_rtype === RT_FIX && uop.lrs2 === 0.U),
      "[rename] x0 is busy??"
    )
  }
  ...
  for (w <- 0 until plWidth) {
    ...
    val current_uop_tainted =
      (s1_tainted || s2_tainted || is_veto_op) && ren2_valids(w)
...
class PredRenameStage(plWidth: Int, numWbPorts: Int)(implicit p: Parameters)
    extends AbstractRenameStage(plWidth, numWbPorts)(p) {
  ...
  for (w <- 0 until plWidth) {
    ...
    next_ftq_idx = Mux(
      ren1_uops(w).is_sfb_br && ren1_fire(w),
      ren1_uops(w).ftq_idx,
      next_ftq_idx
    )
  }
  ...
  for (w <- 0 until plWidth) {
    ...
    io.ren2_uops(w) := BypassAllocations(
      ren2_uops(w),
      ren2_uops.take(w),
      ren2_alloc_reqs.take(w)
    )
    ...
    ren2_uops(w).ppred_busy := ren2_uops(w).is_sfb_shadow && busy_table(
      ren2_uops(w).ppred
    ) && !unbusy(ren2_uops(w).ppred)
  }
...
class ImmRenameStage(plWidth: Int, numWbPorts: Int)(implicit p: Parameters)
    extends AbstractRenameStage(plWidth, numWbPorts)(p) {
  ...
  for (w <- 0 until plWidth) {
    ...
    assert(
      !ren2_alloc_fire(w) || ren2_uops(w).iq_type(IQ_ALU) || ren2_uops(w)
        .iq_type(IQ_MEM) || ren2_uops(w).iq_type(IQ_UNQ)
    )
