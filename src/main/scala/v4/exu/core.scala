...
/** Top level core object that connects the Frontend to the rest of the
  * pipeline.
  */
class BoomCore(roccCSRs: Seq[Seq[CustomCSR]])(implicit p: Parameters)
    extends BoomModule
  ...
  // PERF: Removed the following because it was interfering with the loop near line 222
  ...
  val numImmReaders =
    aluWidth + memWidth + 1 // "Wakeup" immediates when they are read

  val decode_units = (0 until decodeWidth) map { w =>
    Module(new DecodeUnit).suggestName(s"decode_${w}")
  }
  val dec_brmask_logic = Module(new BranchMaskGenerationLogic(coreWidth))
  val rename_stage = Module(
    new RenameStage(coreWidth, numIntPhysRegs, numIntWakeups, false)
  )
  val fp_rename_stage = Module(
    new RenameStage(coreWidth, numFpPhysRegs, numFpWakeupPorts, true)
  )
  val pred_rename_stage = Module(new PredRenameStage(coreWidth, 1))
  val imm_rename_stage = Module(
    new ImmRenameStage(coreWidth, numImmReaders)
  ) // wakeup ports used when insts read imm
  val rename_stages = Seq(
    rename_stage,
    pred_rename_stage,
    imm_rename_stage
  ) ++ (if (usingFPU) Seq(fp_rename_stage) else Nil)
  ...
  val iregfileBankedWriteArray = Seq.fill(lsuWidth + 1) {
    ...
    if (enableColumnALUWrites) Some(w) else None
  })
  val iregfile = Module(
    new BankedRF(
      UInt(xLen.W),
  ...
  /*for (i <- 0 until fp_pipeline.io.wakeups.length) {
    fp_pipeline.io
      .wakeups(i)
      .valid := false.B // Force a single driver starting point
  ...
  for (i <- 0 until fp_wakeup_shadow.length) { // FIXME: SHOULD IT BE numVetoWakeups instead of fp_pipeline.io.wakeups.length????
    ...
    sidecar_unit.io.enq(i).bits := int_wakeups(i).bits
    fp_wakeup_shadow(i).valid := wake_valid && !is_tainted
    fp_wakeup_shadow(i).bits := int_wakeups(i).bits
  }

  fp_pipeline.io.wakeups <> fp_wakeup_shadow
  ...
  pred_wakeup.bits.uop.pdst := Mux1H(
    pred_wakeups.map(_.valid),
    pred_wakeups.map(_.bits.uop.pdst)
  )

  val int_bypasses = Wire(
    Vec(coreWidth + lsuWidth, Valid(new ExeUnitResp(xLen)))
  )
  ...
  for ((b, a) <- brinfos zip alu_exe_units) {
    ...
    b.valid := a.io_brinfo.valid && !rob.io.flush.valid && !IsKilledByBranch(
      brupdate,
      RegNext(rob.io.flush.valid),
      a.io_brinfo.bits
    )
  }
  b1.resolve_mask := brinfos
    .map(x => x.valid << x.bits.uop.br_tag)
    .reduce(_ | _)
  b1.mispredict_mask := brinfos
    .map(x => (x.valid && x.bits.mispredict) << x.bits.uop.br_tag)
    .reduce(_ | _)

  // Find the oldest mispredict and use it to update indices
  val live_brinfos = brinfos.map(br =>
    br.valid && br.bits.mispredict && !IsKilledByBranch(
      brupdate,
      RegNext(rob.io.flush.valid),
      br.bits.uop
    )
  ...
  b2.uop := UpdateBrMask(
    brupdate,
    PriorityMux(live_brinfos, brinfos.map(_.bits.uop))
  )
  ...
  val oldest_mispredict_ftq_idx =
    Mux1H(live_brinfos, brinfos.map(_.bits.uop.ftq_idx))
...
// ("branch resolved",                   () => b2.valid)
  ...
  when(csr.io.rw.cmd =/= freechips.rocketchip.rocket.CSR.N) {
    when(csr.io.rw.addr === 0x800.U) {
      // FIXME: tf ts not working
      // csr.io.rw.rdata := reg_veto_enable.asUInt
      when(csr.io.rw.cmd.isOneOf(CSR.W, CSR.S, CSR.C)) {
        reg_veto_enable := csr.io.rw.wdata(0)
      }
    }
      .elsewhen(csr.io.rw.addr === 0x801.U) {
        // FIXME: same as above
        // csr.io.rw.rdata := reg_veto_threshold // This should be 16-bit, Chisel will zero-extend
        when(csr.io.rw.cmd.isOneOf(CSR.W, CSR.S, CSR.C)) {
  ...
  when(RegNext(rob.io.flush.valid)) {
    ...
    when(FlushTypes.useCsrEvec(flush_typ)) {
      ...
      val flush_pc =
        (AlignPCToBoundary(io.ifu.rrd_ftq_resps(0).pc, icBlockBytes)
  ...
  for (i <- 0 until aluWidth) {
    for (w <- 0 until 2) {
      ...
      for (j <- 0 until 2) {
        ...
        ftq_port_addrs(j) = ftq_port_addrs(j) | Mux(
          was_port_issued_yet || !use_port,
          0.U,
          req.bits
        )
      ...
      alu_exe_units(i).io_rrd_ftq_resps(w) := Mux(
        RegNext(data_sel(0)),
        io.ifu.rrd_ftq_resps(1),
        io.ifu.rrd_ftq_resps(2)
      )
  ...
  val dec_xcpt_stall = (0 until coreWidth).map(w =>
    dec_xcpts(w) &&
      (!rob.io.empty || !io.lsu.fencei_rdy || dec_prior_slot_valid(
        w
      ) || dis_valids.reduce(_ || _) || !xcpt_pc_req.ready)
  )
  ...
  /** TODO This is a bit nasty, but it's currently necessary to split the INT/FP
    * rename pipelines into separate instantiations. Won't have to do this
    * anymore with a properly decoupled FP pipeline.
    */
  for (w <- 0 until coreWidth) {
    ...
    dis_uops(w).pdst := Mux(
      ...
      f_uop.pdst,
      Mux(
        dis_uops(w).dst_rtype === RT_FIX,
        i_uop.pdst,
        Mux(
          dis_uops(w).is_sfb_br,
          p_uop.pdst,
          random.LFSR(
            maxPregSz
          ) // Random set the PDST so the ALU banked IQ is balanced
        )
      )
    )
    ...
    ren_stalls(w) := rename_stage.io.ren_stalls(
      w
    ) || f_stall || p_stall || imm_stall
  }
  ...
  val rocc_shim_busy =
    if (usingRoCC) !unq_exe_unit.io_rocc_core.get.rxq_empty else false.B
  ...
  val rxq_full =
    if (usingRoCC) unq_exe_unit.io_rocc_core.get.rxq_full else false.B
  val block_rocc = (dis_uops zip dis_valids)
    .map { case (u, v) => v && u.is_rocc }
    .scanLeft(rxq_full)(_ || _)
  val dis_rocc_alloc_stall = (dis_uops.map(_.is_rocc) zip block_rocc) map {
    case (p, r) =>
  ...
  val block_brtag = (dis_uops zip dis_valids)
    .map { case (u, v) => v && u.allocate_brtag }
    .scanLeft(false.B)(_ || _)
  val brtag_stall = (dis_uops.map(_.allocate_brtag) zip block_brtag) map {
    case (p, r) =>
  ...
  for (i <- 0 until issueParams.size) {
    ...
    } else if (issueParams(i).iqType == IQ_VETO) {
      val dis_port = dispatcher.io
        .dis_uops(i)(0)
        .asInstanceOf[chisel3.util.DecoupledIO[MicroOp]]
      sidecar_unit.io.dis_uops.valid := dis_port.valid
      sidecar_unit.io.dis_uops.bits := dis_port.bits
      dis_port.ready := sidecar_unit.io.dis_uops.ready
    } else {
      require(false)
    }
  }
  ...
  for (w <- 0 until coreWidth) {
    ...
    immregfile.io.write_ports(w).valid := RegNext(dis_fire(w)) && !uop.imm_sel
      .isOneOf(IS_N, IS_SH)
    immregfile.io.write_ports(w).bits.addr := uop.pimm
    immregfile.io.write_ports(w).bits.data := uop.imm_packed

    bregfile.io.write_ports(w).valid := RegNext(
      dis_fire(w)
    ) && uop.allocate_brtag
  ...
  for (i <- 0 until lsuWidth) {
    ...
    rob.io.wb_resps(wb_idx) := RegNext(
      UpdateBrMask(brupdate, RegNext(rob.io.flush.valid), io.lsu.iresp(i))
    )
    iregfile.io.write_ports(wb_idx).valid := RegNext(io.lsu.iresp(i).valid)
    iregfile.io.write_ports(wb_idx).bits.addr := RegNext(
      io.lsu.iresp(i).bits.uop.pdst
    )
    iregfile.io.write_ports(wb_idx).bits.data := RegNext(
      io.lsu.iresp(i).bits.data
    )
  ...
  if (unq_exe_unit.hasCSR) {
    ...
    if (enableBPDHPMs) {
      ...
      for (i <- 0 until 5) {
        when(
          csr.io.rw.addr === hpmcounter.U || csr.io.rw.addr === mhpmcounter.U
        ) {
          rdata := debug_brs(i)
        }
  ...
  int_wakeups(
    wu_idx
  ).valid := ll_arb.io.out.valid && ll_arb.io.out.bits.uop.dst_rtype === RT_FIX
  ...
  rob.io.wb_resps(wb_idx).valid := RegNext(
    ll_arb.io.out.valid && !IsKilledByBranch(
      brupdate,
      RegNext(rob.io.flush.valid),
      ll_arb.io.out.bits
    )
  ...
  for (i <- 0 until aluWidth) {
    ...
    if (i == 0) {
      ...
      when(unit.io_alu_resp.valid && unit.io_alu_resp.bits.uop.is_tainted) {
        printf(
          ...
          sidecar_unit.io.iss_resps.rs1_data,
          sidecar_unit.io.iss_resps.rs2_data,
    ...
    int_bypasses(
      bypass_idx
    ).valid := unit.io_alu_resp.valid && unit.io_alu_resp.bits.uop.dst_rtype === RT_FIX
    ...
    rob.io.wb_resps(wb_idx).valid := RegNext(
      unit.io_alu_resp.valid && !IsKilledByBranch(
        brupdate,
        RegNext(rob.io.flush.valid),
        unit.io_alu_resp.bits
      )
    )
    ...
    if (i == 0) {
      ...
      when(sidecar_merger.io.out_wb.valid) {
        ...
        iregfile.io
          .write_ports(wb_idx)
          .bits
          .addr := sidecar_merger.io.out_wb.bits.uop.pdst
        iregfile.io
          .write_ports(wb_idx)
          .bits
          .data := sidecar_merger.io.out_wb.bits.data
      }.otherwise {
        // Original ALU logic
        iregfile.io
          .write_ports(wb_idx)
        ...
        iregfile.io
          .write_ports(wb_idx)
          .bits
          .addr := unit.io_alu_resp.bits.uop.pdst
  ...
  val pregfile_write_valids = alu_exe_units.map(u =>
    u.io_alu_resp.valid && u.io_alu_resp.bits.uop.is_sfb_br
  )
  assert(PopCount(pregfile_write_valids) <= 1.U)
  pregfile.io.write_ports(0).valid := pregfile_write_valids.reduce(_ || _)
  pregfile.io.write_ports(0).bits.addr := Mux1H(
    pregfile_write_valids,
    alu_exe_units.map(_.io_alu_resp.bits.uop.pdst)
  )
  pregfile.io.write_ports(0).bits.data := Mux1H(
    pregfile_write_valids,
    alu_exe_units.map(_.io_alu_resp.bits.data)
  )
  ...
  rename_stage.io.child_rebusys := alu_exe_units
    .map(_.io_child_rebusy)
    .reduce(_ | _)
  ...
  for (iss_unit <- Seq(mem_iss_unit, alu_iss_unit, unq_iss_unit)) {
    ...
    iss_unit.io.child_rebusys := alu_exe_units
      .map(_.io_child_rebusy)
      .reduce(_ | _)

    val unit_wakeup_width = iss_unit.io.wakeup_ports.length
    val temp_wakeups = Wire(Vec(unit_wakeup_width, Valid(new Wakeup)))

    for (i <- 0 until unit_wakeup_width) {
      if (i < int_wakeups.length) {
        temp_wakeups(i) := int_wakeups(i)
      } else {
        // This handles cases where the unit expects more than we have
        temp_wakeups(i).valid := false.B
        temp_wakeups(i).bits := 0.U.asTypeOf(new Wakeup)
      }
    }
    iss_unit.io.wakeup_ports := temp_wakeups
  ...
  mem_iss_unit.io.iss_uops zip mem_exe_units map { case (i, u) =>
    u.io_iss_uop := i
  }
  alu_iss_unit.io.iss_uops zip alu_exe_units map { case (i, u) =>
    u.io_iss_uop := i
  }
  unq_iss_unit.io.iss_uops zip unq_exe_units map { case (i, u) =>
    u.io_iss_uop := i
  }
  ...
  assert(
    !(idle_cycles.value(PlusArg("boom_timeout", 13, width = 5))),
    "Pipeline has hung."
  )
  ...
  if (trace) {
    for (w <- 0 until coreWidth) {
      ...
      io.trace.insns(w).priv := RegNext(
        Cat(RegNext(RegNext(csr.io.status.debug)), csr.io.status.prv)
      )
