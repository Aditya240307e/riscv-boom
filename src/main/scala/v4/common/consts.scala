...
trait ScalarOpConstants {
  ...
  def NullMicroOp(implicit p: Parameters) =
    0.U.asTypeOf(new boom.v4.common.MicroOp)
}
...
trait RISCVConstants {
  ...
  def ComputeBranchTarget(pc: UInt, inst: UInt, xlen: Int)(implicit
      p: Parameters
  ): UInt = {
    val b_imm32 =
  ...
  // Note: Accepts only EXPANDED rvc instructions
  def ComputeJALTarget(pc: UInt, inst: UInt, xlen: Int)(implicit
      p: Parameters
  ): UInt = {
    val j_imm32 = Cat(
      Fill(12, inst(31)),
