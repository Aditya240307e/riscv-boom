...
class MicroOp(implicit p: Parameters)
  ...
  val iw_issued =
    Bool() // Was this uop issued last cycle? If so, it can vacate this cycle
  ...
  val dis_col_sel = UInt(
    coreWidth.W
  ) // If using column-issue ALUs with 1-wide dispatch, which column to issue to?
  ...
  def is_sfb_br =
    br_type =/= B_N && is_sfb && enableSFBOpt.B // Does this write a predicate
  def is_sfb_shadow =
    br_type === B_N && is_sfb && enableSFBOpt.B // Is this predicated
  val ldst_is_rs1 =
