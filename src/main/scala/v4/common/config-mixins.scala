...
class WithNBoomPerfCounters(n: Int)
    extends Config((site, here, up) => { case TilesLocated(InSubsystem) =>
      up(TilesLocated(InSubsystem), site) map {
        case tp: BoomTileAttachParams =>
          tp.copy(tileParams =
...
class WithAsynchronousBoomTiles
    extends Config((site, here, up) => { case TilesLocated(InSubsystem) =>
      up(TilesLocated(InSubsystem), site) map {
        case tp: BoomTileAttachParams =>
          tp.copy(crossingParams =
...
/** 2-wide BOOM.
  */
class WithNMediumBooms(n: Int = 1)
    extends Config(
      new WithTAGELBPD ++ // Default to TAGE-L BPD
        new Config((site, here, up) => {
          case TilesLocated(InSubsystem) => {
...
class WithNMegaTapeoutBooms(n: Int = 1, singlePorted: Boolean = true)
    extends Config(
    ...
    cloneLocation: HierarchicalLocation = InSubsystem
      ...
      case TilesLocated(`location`) => {
        ...
        val tileAttachParams = up(TilesLocated(cloneLocation))
          ...
          .get
          .asInstanceOf[BoomTileAttachParams]
        (0 until n).map { i =>
...
/** BOOM Configs for CS152 lab
  */
class WithNCS152BaselineBooms(n: Int = 1)
    extends Config(
      new WithTAGELBPD ++ // Default to TAGE-L BPD
        new Config((site, here, up) => {
          case TilesLocated(InSubsystem) => {
...
class WithNCS152DefaultBooms(n: Int = 1)
    extends Config(
      new WithTAGELBPD ++ // Default to TAGE-L BPD
        new Config((site, here, up) => {
          case TilesLocated(InSubsystem) => {
...
class WithFastTAGEBPD(singlePorted: Boolean = true)
    extends Config((site, here, up) => { case TilesLocated(InSubsystem) =>
      up(TilesLocated(InSubsystem), site) map {
        case tp: BoomTileAttachParams =>
          tp.copy(tileParams =
              ...
              tp.tileParams.core.copy(
                ...
                branchPredictor = (
                    (resp_in: BranchPredictionBankResponse, p: Parameters) => {
                      ...
                      val slowbtb = Module(
                        new SlowBTBBranchPredictorBank(
                          BoomSlowBTBParams(singlePorted = singlePorted)
                        )(p)
                      )
                      val fastbtb = Module(
                        new BTBBranchPredictorBank(
                          BoomBTBParams(
                            nSets = 32,
                            nWays = 2,
                            offsetSz = 13,
                      ...
                      val ubtb = Module(
                        new GMicroBTBBranchPredictorBank(BoomGMicroBTBParams())(
                          p
                        )
                      )
                      val preds =
                        Seq(tage, slowbtb, fastbtb, slowbim, fastbim, ubtb)
              ...
              tp.tileParams.core.copy(
                ...
                branchPredictor = (
                    (resp_in: BranchPredictionBankResponse, p: Parameters) => {
                      ...
                      val lbim = Module(
                        new HBIMBranchPredictorBank(
                          BoomHBIMParams(useLocal = true)
                        )(p)
                      )
...
class WithVetoLogic(enable: Boolean = true)
    extends Config((site, here, up) => { case TilesLocated(InSubsystem) =>
      up(TilesLocated(InSubsystem), site) map {
        case tp: BoomTileAttachParams =>
          tp.copy(tileParams =
