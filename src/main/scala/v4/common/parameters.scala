...
// DOC include start: BOOM Parameters
    ...
    branchPredictor: Function2[
      BranchPredictionBankResponse,
      Parameters,
      Tuple2[Seq[BranchPredictorBank], BranchPredictionBankResponse]
    ] = (
    ...
    mulDiv: Option[freechips.rocketchip.rocket.MulDivParams] = Some(
      MulDivParams(divEarlyOut = true)
    ),
...
/** Defines custom BOOM CSRs
  */
class BoomCustomCSRs(implicit p: Parameters)
    extends freechips.rocketchip.tile.CustomCSRs
  ...
  val enableVetoCSRId = 0x809
  def enableVetoCSR = Some(
    CustomCSR(enableVetoCSRId, BigInt(1), Some(BigInt(0)))
  ) // Init to 0 (disabled)

  def marchid = CustomCSR.constant(CSRs.marchid, BigInt(2))

  override def decls =
    enableOOOCSR.toSeq ++ enableBPDCSR.toSeq ++ enableVetoCSR.toSeq ++ bpmCSR.toSeq ++ chickenCSR ++ Seq(
      marchid
    )
  def enableVeto = getOrElse(enableVetoCSR, _.value(0), false.B)
  def enableOOO = getOrElse(enableOOOCSR, _.value(0), true.B) && !getOrElse(
    chickenCSR,
    _.value(3),
    false.B
  )
...
/** Mixin trait to add BOOM parameters to expand other traits/objects/etc
  */
trait HasBoomCoreParameters
    extends freechips.rocketchip.tile.HasCoreParameters {
  ...
  val numRobEntries =
    boomParams.numRobEntries // number of ROB entries (e.g., 32 entries for R10k)
  val numRxqEntries =
    boomParams.numRXQEntries // number of RoCC execute queue entries. Keep small since this holds operands and instruction bits
  val numRcqEntries =
    boomParams.numRCQEntries // number of RoCC commit queue entries. This can be large since it just keeps a pdst
  ...
  val maxBrCount =
    boomParams.maxBrCount // number of branches we can speculate simultaneously
  val ftqSz = boomParams.ftq.nEntries // number of FTQ entries
  val numFetchBufferEntries =
    boomParams.numFetchBufferEntries // number of instructions that stored between fetch&decode

  val numIntPhysRegs =
    boomParams.numIntPhysRegisters // size of the integer physical register file
  val numFpPhysRegs =
    boomParams.numFpPhysRegisters // size of the floating point physical register file
  ...
  val dfmaLatency =
    if (boomParams.fpu.isDefined) boomParams.fpu.get.dfmaLatency else 3
  val sfmaLatency =
    if (boomParams.fpu.isDefined) boomParams.fpu.get.sfmaLatency else 3
  ...
  val enableCompactingLSUDuringDispatch =
    boomParams.enableCompactingLSUDuringDispatch
  ...
  val enableColumnALUWrites =
    boomParams.enableColumnALUIssue && isPow2(aluWidth) && aluWidth > 1
  ...
  val COMMIT_LOG_PRINTF =
    boomParams.enableCommitLogPrintf // dump commit state, for comparision against ISA sim
  val BRANCH_PRINTF =
    boomParams.enableBranchPrintf // dump branch predictor results
  val MEMTRACE_PRINTF =
    boomParams.enableMemtracePrintf // dump trace of memory accesses to L1D for debugging
