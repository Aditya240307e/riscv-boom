...
class BoomDCacheReq(implicit p: Parameters)
  ...
  val is_hella =
    Bool() // Is this the hellacache req? If so this is not tracked in LDQ or STQ
}

class BoomDCacheResp(implicit p: Parameters)
    extends BoomBundle()(p)
    with HasBoomUOP {
  val data = Bits(coreDataBits.W)
  val is_hella = Bool()
...
class LSUDMemIO(implicit p: Parameters, edge: TLEdgeOut)
    extends BoomBundle()(p) {
...
class LDQEntry(implicit p: Parameters) extends BoomBundle()(p) with HasBoomUOP {
  ...
  val addr_is_uncacheable =
    Bool() // Uncacheable, wait until head of ROB to execute
  ...
  val forward_stq_idx = UInt(
    (1 + stqAddrSz).W
  ) // Which store did we get the store-load forward from?

  val debug_wb_data = UInt(xLen.W)
}

class STQEntry(implicit p: Parameters) extends BoomBundle()(p) with HasBoomUOP {
  val addr = Valid(UInt(coreMaxAddrBits.W))
  val addr_is_virtual = Bool() // Virtual address, we got a TLB miss
  ...
  val succeeded =
    Bool() // D$ has ack'd this, we don't need to maintain this anymore
...
class LSU(implicit p: Parameters, edge: TLEdgeOut)
  ...
  val ldq_next_stq_idx = Reg(
    Vec(numLdqEntries, UInt((stqAddrSz + 1).W))
  ) // the one bit is for overflow calculation
  ...
  val ldq_veto_stall = Reg(Vec(numLdqEntries, Bool()))
  ...
  val stq_head = Reg(
    UInt((stqAddrSz + 1).W)
  ) // point to next store to clear from STQ (i.e., send to memory)
  val stq_tail = Reg(UInt((stqAddrSz + 1).W))
  val stq_commit_head = Reg(
    UInt((stqAddrSz + 1).W)
  ) // point to next store to commit
  val stq_execute_head = Reg(
    UInt((stqAddrSz + 1).W)
  ) // point to next store to execute

  val h_ready :: h_s1 :: h_s2 :: h_s2_nack :: h_wait :: h_replay :: h_dead :: Nil =
    Enum(7)
  ...
  var next_live_store_mask =
    Mux(clear_store, live_store_mask & ~(1.U << stq_head), live_store_mask)
  ...
  for (w <- 0 until coreWidth) {
    ...
    val dis_ld_val =
      io.core.dis_uops(w).valid && io.core.dis_uops(w).bits.uses_ldq && !io.core
        .dis_uops(w)
        .bits
        .exception
    val dis_st_val =
      io.core.dis_uops(w).valid && io.core.dis_uops(w).bits.uses_stq && !io.core
        .dis_uops(w)
        .bits
        .exception
    ...
    when(dis_ld_val) {
      ...
      assert(
        actual_ld_enq_idx === io.core.dis_uops(w).bits.ldq_idx,
        "[lsu] mismatch enq load tag."
      )
      assert(
        !ldq_valid(actual_ld_enq_idx),
        "[lsu] Enqueuing uop is overwriting ldq entries"
      )
    }
    when(dis_st_val) {
      ...
      assert(
        actual_st_enq_idx === io.core.dis_uops(w).bits.stq_idx,
        "[lsu] mismatch enq store tag."
      )
      assert(
        !stq_valid(actual_st_enq_idx),
        "[lsu] Enqueuing uop is overwriting stq entries"
      )
    }
    assert(
      !(dis_ld_val && dis_st_val),
      "[lsu] cannot enqueue load and store at the same time into a single pipe!"
    )
    ...
    when(dis_uops(w).valid && dis_uops(w).bits.uses_stq) {
      ...
      stq_valid(stq_idx) := !IsKilledByBranch(
        io.core.brupdate,
        io.core.exception,
        dis_uops(w).bits
      )
    ...
    ld_enq_idx =
      Mux(dis_ld_val, WrapIncWCarry(ld_enq_idx, numLdqEntries), ld_enq_idx)
    actual_ld_enq_idx = (ld_enq_idx)

    next_live_store_mask = Mux(
      dis_st_val,
      next_live_store_mask | (1.U << st_enq_idx),
      next_live_store_mask
    )
    st_enq_idx =
      Mux(dis_st_val, WrapIncWCarry(st_enq_idx, numStqEntries), st_enq_idx)
    actual_st_enq_idx = (st_enq_idx)

    assert(
      !(dis_ld_val && dis_st_val),
      "A UOP is trying to go into both the LDQ and the STQ"
    )

    ldq_tail_oh = Mux(
      (io.core.dis_uops(w).bits.uses_ldq && !io.core
        .dis_uops(w)
        .bits
        .exception) || !enableCompactingLSUDuringDispatch.B,
      RotateL1(ldq_tail_oh),
      ldq_tail_oh
    )
    stq_tail_oh = Mux(
      (io.core.dis_uops(w).bits.uses_stq && !io.core
        .dis_uops(w)
        .bits
        .exception) || !enableCompactingLSUDuringDispatch.B,
      RotateL1(stq_tail_oh),
      stq_tail_oh
    )
  }
  ...
  for (i <- 0 until numLdqEntries) {
    ...
    when(
      io.core.exception || IsKilledByBranch(
        io.core.brupdate,
        io.core.exception,
        ldq_uop(i)
      )
  ...
  val block_load_mask = WireInit(
    VecInit((0 until numLdqEntries).map(x => false.B))
  )
  ...
  val stq_tail_plus = WrapAddWCarryWidth(
    stq_tail,
    (2 * coreWidth).U,
    2 * numStqEntries,
    (1 + stqAddrSz)
  )
  val stq_almost_full = SafeRegNext(
    IsOlderLSU(stq_head, stq_tail_plus, stq_tail)
  )
  ...
  val ldq_wakeup_idx = SafeRegNext(
    LSUAgePriorityEncoder(
      (0 until numLdqEntries).map(i => {
        val block = block_load_mask(i) || p1_block_load_mask(i)
        ldq_addr(i).valid && !ldq_executed(i) && !ldq_succeeded(
          i
        ) && !ldq_addr_is_virtual(i) && !block
      }),
      ldq_head
    )
  )
  ...
  ldq_enq_retry_idx := LSUAgePriorityEncoder(
    (0 until numLdqEntries).map(i => {
      ldq_addr(i).valid && ldq_addr_is_virtual(i) && (i.U =/= GetRealLSQIdx(
        ldq_enq_retry_idx
      ))
    }),
    ldq_head
  ...
  stq_enq_retry_idx := LSUAgePriorityEncoder(
    (0 until numStqEntries).map(i => {
      stq_addr(i).valid && stq_addr_is_virtual(i) && (i.U =/= GetRealLSQIdx(
        stq_enq_retry_idx
      ))
    }),
    stq_commit_head
  ...
  retry_queue.io.enq.bits.data := Mux(
    can_enq_store_retry,
    stq_enq_retry_e.bits.addr.bits,
    ldq_enq_retry_e.bits.addr.bits
  )
  retry_queue.io.enq.bits.uop := Mux(
    can_enq_store_retry,
    stq_enq_retry_e.bits.uop,
    ldq_enq_retry_e.bits.uop
  )
  ...
  retry_queue.io.deq.ready := will_fire_load_retry.reduce(
    _ || _
  ...
  stq_execute_queue.io.deq.ready := will_fire_store_commit_fast.reduce(
    _ || _
  ...
  val can_fire_load_agen_exec =
    widthMap(w => agen(w).valid && agen(w).bits.uop.uses_ldq)
  val can_fire_load_agen = can_fire_load_agen_exec

  // Can we fire an incoming store addrgen + store datagen
  val can_fire_store_agen =
    widthMap(w => agen(w).valid && agen(w).bits.uop.uses_stq)
  ...
  val can_fire_release =
    widthMap(w => (w == lsuWidth - 1).B && io.dmem.release.valid)
  ...
  val can_fire_store_commit_fast =
    widthMap(w => can_fire_store_commit_slow(w) && stq_almost_full)
  ...
  val can_fire_hella_incoming = WireInit(
    widthMap(w => false.B)
  ) // This is assigned to in the hellashim ocntroller

  // Can we fire a hellacache request that the dcache nack'd
  val can_fire_hella_wakeup = WireInit(
    widthMap(w => false.B)
  ) // This is assigned to in the hellashim controller
  ...
  for (w <- 0 until lsuWidth) {
    ...
    def lsu_sched(
      ...
      dontTouch(
        will_fire
      ) // dontTouch these so we can inspect the will_fire signals
      will_fire
    }
    ...
    will_fire_sfence(w) := lsu_sched(
      can_fire_sfence(w),
      true,
      false,
      false
    ) // TLB ,    ,
    will_fire_store_commit_fast(w) := lsu_sched(
      can_fire_store_commit_fast(w),
      false,
      true,
      false
    ) //     , DC          If store queue is filling up, prioritize draining it
    will_fire_load_agen_exec(w) := lsu_sched(
      can_fire_load_agen_exec(w),
      true,
      true,
      true
    ) // TLB , DC , LCAM   Normally fire loads as soon as translation completes
    will_fire_load_agen(w) := lsu_sched(
      can_fire_load_agen(w),
      true,
      false,
      true
    ) // TLB ,    , LCAM   If we are draining stores, still translate the loads
    will_fire_store_agen(w) := lsu_sched(
      can_fire_store_agen(w),
      true,
      false,
      true
    ) // TLB ,    , LCAM
    will_fire_release(w) := lsu_sched(
      can_fire_release(w),
      false,
      false,
      true
    ) //            LCAM
    will_fire_hella_incoming(w) := lsu_sched(
      can_fire_hella_incoming(w),
      true,
      true,
      false
    ) // TLB , DC
    will_fire_hella_wakeup(w) := lsu_sched(
      can_fire_hella_wakeup(w),
      false,
      true,
      false
    ) //     , DC
    will_fire_store_retry(w) := lsu_sched(
      can_fire_store_retry(w),
      true,
      false,
      true
    ) // TLB ,    , LCAM
    will_fire_load_retry(w) := lsu_sched(
      can_fire_load_retry(w),
      true,
      true,
      true
    ) // TLB , DC , LCAM
    will_fire_load_wakeup(w) := lsu_sched(
      can_fire_load_wakeup(w),
      false,
      true,
      true
    ) //     , DC , LCAM
    will_fire_store_commit_slow(w) := lsu_sched(
      can_fire_store_commit_slow(w),
      false,
      true,
      false
    ) //     , DC
    ...
    assert(
      !(agen(w).valid && !(will_fire_load_agen_exec(w) || will_fire_load_agen(
        w
  ...
  assert(
    !(hella_state =/= h_ready && hella_req.cmd === rocket.M_SFENCE),
  ...
  val exe_tlb_uop = widthMap(w =>
    Mux(
      ...
      ldq_incoming_e(w).bits.uop,
      Mux(
        will_fire_store_agen(w),
        stq_incoming_e(w).bits.uop,
        Mux(
          will_fire_load_retry(w) ||
            will_fire_store_retry(w),
          retry_queue.io.deq.bits.uop,
          Mux(
  ...
  val exe_tlb_vaddr = widthMap(w =>
    Mux(
      ...
      Mux(
        will_fire_sfence(w),
        io.core.sfence.bits.addr,
        Mux(
          will_fire_load_retry(w) ||
            will_fire_store_retry(w),
          retry_queue.io.deq.bits.data,
          Mux(will_fire_hella_incoming(w), hella_req.addr, 0.U)
        )
      )
    )
  )
  ...
  val exe_size = widthMap(w =>
    Mux(
      ...
      Mux(will_fire_hella_incoming(w), hella_req.size, 0.U)
    )
  )
  val exe_cmd = widthMap(w =>
    Mux(
      will_fire_load_agen_exec(w) ||
        ...
        will_fire_load_retry(w) ||
        will_fire_store_retry(w),
      exe_tlb_uop(w).mem_cmd,
      Mux(
        will_fire_hella_incoming(w),
        hella_req.cmd,
        Mux(will_fire_sfence(w), rocket.M_SFENCE, 0.U)
      )
    )
  )

  val exe_passthr =
    widthMap(w => Mux(will_fire_hella_incoming(w), hella_req.phys, false.B))
  val exe_kill = widthMap(w =>
    Mux(will_fire_hella_incoming(w), io.hellacache.s1_kill, false.B)
  )
  val bkptu = Seq.fill(lsuWidth) {
    Module(new rocket.BreakpointUnit(nBreakpoints))
  }
  for (w <- 0 until lsuWidth) {
  ...
  val ma_st = widthMap(w =>
    dtlb.io.resp(w).ma.st && exe_tlb_uop(w).uses_stq && !exe_tlb_uop(w).is_fence
  )
  ...
  val dbg_bp = widthMap(w =>
    bkptu(w).io.debug_st && ((exe_tlb_uop(w).uses_ldq && bkptu(
      w
    ).io.debug_ld) ||
      (exe_tlb_uop(w).uses_stq && bkptu(w).io.debug_st && !exe_tlb_uop(
        w
      ).is_fence))
  )
  val bp = widthMap(w =>
    bkptu(w).io.debug_st && ((exe_tlb_uop(w).uses_ldq && bkptu(w).io.xcpt_ld) ||
      (exe_tlb_uop(w).uses_stq && bkptu(w).io.xcpt_st && !exe_tlb_uop(
        w
      ).is_fence))
  )

  // TODO check for xcpt_if and verify that never happens on non-speculative instructions.
  val mem_xcpt_valids = RegNext(
    widthMap(w =>
      exe_tlb_valid(w) &&
        (pf_ld(w) || pf_st(w) || ae_ld(w) || ae_st(w) || ma_ld(w) || ma_st(
          w
        ) || dbg_bp(w) || bp(w)) &&
        !IsKilledByBranch(io.core.brupdate, io.core.exception, exe_tlb_uop(w))
    )
  )
  val mem_xcpt_uops = RegNext(
    widthMap(w => UpdateBrMask(io.core.brupdate, exe_tlb_uop(w)))
  )
  val mem_xcpt_causes = RegNext(
    widthMap(w =>
      Mux(
        dbg_bp(w),
        rocket.CSR.debugTriggerCause.U,
        Mux(
          bp(w),
          rocket.Causes.breakpoint.U,
          Mux(
            ma_st(w),
            rocket.Causes.misaligned_store.U,
            Mux(
              ma_ld(w),
              rocket.Causes.misaligned_load.U,
              Mux(
                pf_st(w),
  ...
  for (w <- 0 until lsuWidth) {
    assert(
      !(dtlb.io.req(w).valid && exe_tlb_uop(w).is_fence),
      "Fence is pretending to talk to the TLB"
    )
  }
  ...
  for (w <- 1 until lsuWidth) {
    ...
    oldest_xcpt_rob_idx =
      Mux(is_older, mem_xcpt_uops(w).rob_idx, oldest_xcpt_rob_idx)
  }

  val exe_tlb_miss = widthMap(w =>
    dtlb.io.req(w).valid && (dtlb.io.resp(w).miss || !dtlb.io.req(w).ready)
  )
  val exe_tlb_paddr = widthMap(w =>
    Cat(
      dtlb.io.resp(w).paddr(paddrBits - 1, corePgIdxBits),
  ...
  for (w <- 0 until lsuWidth) {
    assert(
      exe_tlb_paddr(w) === dtlb.io.resp(w).paddr,
      "[lsu] paddrs should match."
    )

    when(mem_xcpt_valids(w)) {
      assert(
        RegNext(
          will_fire_load_agen_exec(w) || will_fire_load_agen(
            w
          ) || will_fire_store_agen(w) ||
  ...
  val exe_agen_killed = widthMap(w =>
    IsKilledByBranch(io.core.brupdate, io.core.exception, agen(w).bits.uop)
  )
  ...
  val s0_executing_loads = WireInit(
    VecInit((0 until numLdqEntries).map(x => false.B))
  )
  ...
  for (w <- 0 until lsuWidth) {
    ...
    when(will_fire_load_agen_exec(w)) {
      ...
      s0_kills(w) := exe_tlb_miss(w) || exe_tlb_uncacheable(w) || ma_ld(
        w
      ) || ae_ld(w) || pf_ld(w)
      s0_executing_loads(ldq_incoming_idx(w)) := dmem_req_fire(w) && !s0_kills(
        w
      )
      ...
      will_fire_store_commit_slow(w) || will_fire_store_commit_fast(w)
    ) {
      dmem_req(w).valid := true.B
      ...
      dmem_req(w).valid := !io.hellacache.s1_kill && (!exe_tlb_miss(
        w
      ) || hella_req.phys)
      dmem_req(w).bits.addr := exe_tlb_paddr(w)
      dmem_req(w).bits.data := (new freechips.rocketchip.rocket.StoreGen(
    ...
    when(
      will_fire_load_agen(w) || will_fire_load_agen_exec(
        w
      ) || will_fire_load_retry(w)
    ) {
      val ldq_idx = Mux(
        will_fire_load_agen(w) || will_fire_load_agen_exec(w),
        ldq_incoming_idx(w),
        ldq_retry_idx
      )
      ldq_addr(ldq_idx).valid := !exe_agen_killed(w) || will_fire_load_retry(w)
      ldq_addr(ldq_idx).bits := Mux(
        exe_tlb_miss(w),
        exe_tlb_vaddr(w),
        exe_tlb_paddr(w)
      )
      ldq_ld_byte_mask(ldq_idx) := GenByteMask(
        exe_tlb_vaddr(w),
        exe_tlb_uop(w).mem_size
      )
      ldq_uop(ldq_idx).pdst := exe_tlb_uop(w).pdst
      ldq_addr_is_virtual(ldq_idx) := exe_tlb_miss(w)
      ldq_addr_is_uncacheable(ldq_idx) := exe_tlb_uncacheable(
        w
      ) && !exe_tlb_miss(w)

      assert(
        !ldq_addr(ldq_idx).valid,
    ...
    when(will_fire_store_agen(w) || will_fire_store_retry(w)) {
      val stq_idx =
        Mux(will_fire_store_agen(w), stq_incoming_idx(w), stq_retry_idx)

      stq_addr(stq_idx).valid := (!exe_agen_killed(w) || will_fire_store_retry(
        w
      )) && !pf_st(w) // Prevent AMOs from executing!
      stq_addr(stq_idx).bits := Mux(
        exe_tlb_miss(w),
        exe_tlb_vaddr(w),
        exe_tlb_paddr(w)
      )
  ...
  val fired_load_agen_exec =
    widthMap(w => RegNext(will_fire_load_agen_exec(w) && !exe_agen_killed(w)))
  val fired_load_agen =
    widthMap(w => RegNext(will_fire_load_agen(w) && !exe_agen_killed(w)))
  val fired_store_agen =
    widthMap(w => RegNext(will_fire_store_agen(w) && !exe_agen_killed(w)))
  val fired_sfence = RegNext(will_fire_sfence)
  val fired_release = RegNext(will_fire_release)
  val fired_load_retry = widthMap(w =>
    RegNext(
      will_fire_load_retry(w) && !IsKilledByBranch(
        io.core.brupdate,
        io.core.exception,
        retry_queue.io.deq.bits.uop
      )
  ...
  val fired_store_retry = widthMap(w =>
    RegNext(
      will_fire_store_retry(w) && !IsKilledByBranch(
        io.core.brupdate,
        io.core.exception,
        retry_queue.io.deq.bits.uop
      )
  ...
  val fired_store_commit = widthMap(w =>
    RegNext(will_fire_store_commit_slow(w) || will_fire_store_commit_fast(w))
  )
  val fired_load_wakeup = widthMap(w =>
    RegNext(
      will_fire_load_wakeup(w) && !IsKilledByBranch(
        io.core.brupdate,
        io.core.exception,
        ldq_wakeup_e.bits.uop
      )
    )
  )
  ...
  val mem_incoming_uop = SafeRegNext(
    widthMap(w => UpdateBrMask(io.core.brupdate, agen(w).bits.uop))
  )
  val mem_ldq_incoming_e = RegNext(
    widthMap(w =>
      UpdateBrMask(io.core.brupdate, io.core.exception, ldq_incoming_e(w))
    )
  )
  val mem_stq_incoming_e = RegNext(
    widthMap(w =>
      UpdateBrMask(io.core.brupdate, io.core.exception, stq_incoming_e(w))
    )
  )
  val mem_ldq_wakeup_e = RegNext(
    UpdateBrMask(io.core.brupdate, io.core.exception, ldq_wakeup_e)
  )
  val mem_ldq_retry_e = RegNext(
    UpdateBrMask(io.core.brupdate, io.core.exception, ldq_read(ldq_retry_idx))
  )
  val mem_stq_retry_e = RegNext(
    UpdateBrMask(io.core.brupdate, io.core.exception, stq_read(stq_retry_idx))
  )
  val mem_ldq_e = widthMap(w =>
    Mux(
      fired_load_agen(w) ||
        fired_load_agen_exec(w),
      mem_ldq_incoming_e(w),
      Mux(
        fired_load_retry(w),
        mem_ldq_retry_e,
        Mux(
  ...
  val stq_clr_head_idx = SafeRegNext(
    LSUAgePriorityEncoder(
      (0 until numStqEntries).map(i => {
  ...
  for (i <- 0 until coreWidth) {
    ...
    val clr_valid_1 = RegNext(
      clr_valid && !IsKilledByBranch(
        io.core.brupdate,
        io.core.exception,
        clr_uop
      )
    ...
    io.core.clr_bsy(i).valid := clr_valid_1 && !IsKilledByBranch(
      io.core.brupdate,
      io.core.exception,
      clr_uop_1
    )
  ...
  val do_st_search = widthMap(w =>
    (fired_store_agen(w) || fired_store_retry(w)) && !mem_tlb_miss(w)
  )
  // We translated a load last cycle
  val do_ld_search = widthMap(w =>
    ((fired_load_agen(w) || fired_load_agen_exec(w) || fired_load_retry(
      w
    )) && !mem_tlb_miss(w)) ||
      fired_load_wakeup(w)
  )
  ...
  val lcam_addr = widthMap(w =>
    Mux(
      fired_store_agen(w) || fired_store_retry(w) || fired_load_agen(
        w
      ) || fired_load_agen_exec(w),
      RegNext(exe_tlb_paddr(w)),
      Mux(fired_release(w), RegNext(io.dmem.release.bits.address), mem_paddr(w))
    )
  )
  val lcam_uop = widthMap(w =>
    Mux(
      do_st_search(w),
      mem_stq_e(w).bits.uop,
  ...
  val lcam_ldq_idx = widthMap(w =>
    Mux(
      fired_load_agen(w) ||
        fired_load_agen_exec(w),
      mem_incoming_uop(w).ldq_idx,
      Mux(
        fired_load_wakeup(w),
        SafeRegNext(ldq_wakeup_idx),
        Mux(
          fired_load_retry(w),
          SafeRegNext(ldq_retry_idx),
          0.U((1 + ldqAddrSz).W)
        )
      )
    )
  ...
  val lcam_stq_idx = widthMap(w =>
    Mux(
      fired_store_agen(w),
      mem_incoming_uop(w).stq_idx,
      Mux(
        fired_store_retry(w),
        SafeRegNext(stq_retry_idx),
        0.U((1 + stqAddrSz).W)
      )
    )
  )
  ...
  for (w <- 0 until lsuWidth) {
    for (i <- 0 until numLdqEntries) {
      lcam_younger_load_mask(w)(i) := EntryValidFromAge(
        lcam_ldq_idx(w),
        ldq_tail,
        i.U((ldqAddrSz + 1).W)
      ) & (i.U =/= GetRealLSQIdx(lcam_ldq_idx(w)))
    }
  }

  val can_forward = widthMap(w =>
    Mux(
      fired_load_agen(w) || fired_load_agen_exec(w) || fired_load_retry(w),
  ...
  val wb_ldst_forward_e = widthMap(w =>
    RegNext(UpdateBrMask(io.core.brupdate, ldq_read(lcam_ldq_idx(w)).bits))
  )
  ...
  for (i <- 0 until numLdqEntries) {
    ...
    val block_addr_matches = widthMap(w =>
      lcam_addr(w) >> blockOffBits === l_addr.bits >> blockOffBits
    )
    val dword_addr_matches = widthMap(w =>
      block_addr_matches(w) && lcam_addr(w)(blockOffBits - 1, 3) === l_addr
        .bits(blockOffBits - 1, 3)
    )
    ...
    for (w <- 0 until lsuWidth) {
      ...
      when(
        ...
        val forwarded_is_older =
          IsOlderLSU(l_forward_stq_idx, lcam_stq_idx(w), l_uop.stq_idx)
        // We are older than this load, which overlapped us.
        when(
          !l_forward_std_val || // If the load wasn't forwarded, it definitely failed
            ((l_forward_stq_idx =/= lcam_stq_idx(w)) && forwarded_is_older)
        ) { // If the load forwarded from us, we might be ok
        ...
        when(searcher_is_older) {
          when(
            (l_executed || l_succeeded) &&
              !s1_executing_loads(
                i
              ) && // If the load is proceeding in parallel we don't need to kill it
              l_observed
          ) { // Its only a ordering failure if the cache line was observed between the younger load and us
            // ldq_order_fail(i) := true.B
            // failed_load := true.B
          ...
          when(!(l_executed && (l_succeeded || l_will_succeed))) {
            ...
            when(
              RegNext(dmem_req_fire(w) && !s0_kills(w)) && !fired_load_agen(w)
            ) {
  ...
  for (wi <- 0 until lsuWidth) {
    for (w <- 0 until lsuWidth) {
      ...
      val nack_dword_addr_matches =
        (lcam_addr(w) >> 3) === (io.dmem.nack(wi).bits.addr >> 3)
      val nack_mask = GenByteMask(
        io.dmem.nack(wi).bits.addr,
        io.dmem.nack(wi).bits.uop.mem_size
      )
      ...
      when(
        do_ld_search(w) &&
          ...
          nack_mask_overlap &&
          IsOlderLSU(
            io.dmem.nack(wi).bits.uop.ldq_idx,
            lcam_ldq_idx(w),
            ldq_head
          )
      ) {
      ...
      val forward_dword_addr_matches =
        (lcam_addr(w) >> 3 === wb_ldst_forward_ld_addr(wi) >> 3)
      val forward_mask = GenByteMask(
        wb_ldst_forward_ld_addr(wi),
        wb_ldst_forward_e(wi).uop.mem_size
      )
      ...
      val forwarded_is_older = IsOlderLSU(
        wb_ldst_forward_stq_idx(wi),
        lcam_stq_idx(w),
        wb_ldst_forward_e(wi).uop.stq_idx
      )
  ...
  for (i <- 0 until numStqEntries) {
    ...
    for (w <- 0 until lsuWidth) {
      val dword_addr_matches = (s_addr.valid &&
        ...
        !s_addr_is_virtual &&
        (s_addr
          .bits(corePAddrBits - 1, 3) === lcam_addr(w)(corePAddrBits - 1, 3)))
      val mask_union = lcam_mask(w) & write_mask

      addr_matches(w)(i) := (mask_union =/= 0.U) && dword_addr_matches
      forward_matches(w)(i) := addr_matches(w)(
        i
      ) && s_data.valid && (mask_union === lcam_mask(w))
      ...
      assert(
        reset.asBool | io.core.brupdate.b1.mispredict_mask.orR | io.core.brupdate.b2.mispredict | s_valid === EntryValidFromAge(
          stq_head,
          stq_tail,
          i.U((stqAddrSz + 1).W)
        ),
        s"Entry $i has failed"
      )

      age_matches(w)(i) := EntryValidFromAge(
        stq_head,
        lcam_next_stq_idx(w),
        i.U((stqAddrSz + 1).W)
      )
  ...
  for (w <- 0 until lsuWidth) {
    ...
    ldst_addr_matches(w) := (addr_matches(w).asUInt & age_matches(
      w
    ).asUInt) & fast_stq_valids
    ldst_forward_matches(w) := (forward_matches(w).asUInt & age_matches(
      w
    ).asUInt) & fast_stq_valids
    stld_prs2_matches(w) := (prs2_matches(w).asUInt & ~age_matches(
      w
    ).asUInt) & fast_stq_valids
  }
  ...
  for (w <- 0 until lsuWidth) {
    val (youngest_matching, match_found) = ForwardingAgeLogic(
      numStqEntries,
      ldst_addr_matches(w),
      GetRealLSQIdx(lcam_uop(w).stq_idx)
    )
    val (youngest_forwarder, forwarder_found) = ForwardingAgeLogic(
      numStqEntries,
    ...
    wb_ldst_forward_valid(w) := (youngest_matching === youngest_forwarder &&
      ...
      !RegNext(
        IsKilledByBranch(io.core.brupdate, io.core.exception, lcam_uop(w))
      ))
  ...
  if (lsuWidth == 1) {
    ...
    when(
      will_fire_store_commit_fast(0) || will_fire_store_commit_slow(
        0
      ) || !can_fire_store_commit_slow(0)
    ) {
      store_blocked_counter := 0.U
    }.elsewhen(
      can_fire_store_commit_slow(0) && !(will_fire_store_commit_slow(
        0
      ) || will_fire_store_commit_fast(0))
    ) {
      store_blocked_counter := Mux(
        store_blocked_counter === 15.U,
        15.U,
        store_blocked_counter + 1.U
      )
    }
  ...
  val mem_stld_forward_stq_idx = widthMap(w =>
    LSUAgePriorityEncoder(stld_prs2_matches(w).asBools, lcam_uop(w).stq_idx)
  )
  val mem_stld_forward_valid = widthMap(w =>
    do_ld_search(w) && stld_prs2_matches(w)(mem_stld_forward_stq_idx(w))
  )
  ...
  for (w <- 0 until lsuWidth) {
    io.core.clr_unsafe(w).valid := (
      RegNext(do_st_search(w)) ||
        (!io.dmem.nack(w).valid && RegNext(
          do_ld_search(w) && !fired_load_agen(w) && !io.dmem
            .s1_kill(w) && RegNext(dmem_req_fire(w))
        ))
    ) && !RegNext(failed_load)
  ...
  val l_idx = LSUAgePriorityEncoder(
    (0 until numLdqEntries).map { i => ldq_valid(i) && ldq_order_fail(i) },
    ldq_head
  )
  val ld_xcpt_uop = WireInit(ldq_uop(l_idx))

  val use_mem_xcpt = (mem_xcpt_valid && IsOlder(
    mem_xcpt_uop.rob_idx,
    ld_xcpt_uop.rob_idx,
    io.core.rob_head_idx
  )) || !ld_xcpt_valid
  ...
  r_xcpt_valid := (ld_xcpt_valid || mem_xcpt_valid) && !IsKilledByBranch(
    io.core.brupdate,
    io.core.exception,
    xcpt_uop
  )
  ...
  io.core.lxcpt.valid := r_xcpt_valid && !IsKilledByBranch(
    io.core.brupdate,
    io.core.exception,
    r_xcpt.uop
  )
  ...
  for (w <- 0 until lsuWidth) {
    ...
    w1.valid := wakeupArbs(w).io.in(1).fire && !IsKilledByBranch(
      io.core.brupdate,
      io.core.exception,
      wakeupArbs(w).io.in(1).bits
    )
    w1.bits := UpdateBrMask(io.core.brupdate, wakeupArbs(w).io.in(1).bits)

    val w2 = Reg(Valid(new Wakeup))
    w2.valid := w1.valid && !IsKilledByBranch(
      io.core.brupdate,
      io.core.exception,
      w1.bits
    )
  ...
  for (w <- 0 until lsuWidth) {
    ...
    // Handle the response
    val resp = Mux(
      !io.dmem.resp(w).valid && (w == lsuWidth - 1).B,
      io.dmem.ll_resp.bits,
      io.dmem.resp(w).bits
    )
    if (w == lsuWidth - 1) {
      io.dmem.ll_resp.ready := !io.dmem.resp(w).valid && !wb_spec_wakeups(
        w
      ).valid
    }
    when(
      io.dmem.resp(w).valid || ((w == lsuWidth - 1).B && io.dmem.ll_resp.fire)
    ) {
    ...
    when(dmem_resp_fired(w) && wb_ldst_forward_valid(w)) {
      ...
      .elsewhen(!dmem_resp_fired(w) && wb_ldst_forward_valid(w)) {
        ...
        val storegen = new freechips.rocketchip.rocket.StoreGen(
          stq_uop(s_idx).mem_size,
          stq_addr(s_idx).bits,
          stq_data(s_idx).bits,
          coreDataBytes
        )
        val trace_storegen_data =
          WireDefault(storegen.data).suggestName("storegen_data")
    ...
    if (enableStLdForwarding) {
      when(
        ...
        stq_data(RegNext(mem_stld_forward_stq_idx(w))).bits := iresp(
          w
        ).bits.data
      }
    }
    ...
    slow_wakeups(w) := (if (enableFastLoadUse) wb_slow_wakeups(w)
                        else
                          RegNext(
                            UpdateBrMask(
                              io.core.brupdate,
    ...
    when(slow_wakeups(w).valid) {
      when(
        spec_wakeups(w).valid
      ) { // Do nothing, since the slow wakeup matches a earlier speculative wakeup
        assert(
          slow_wakeups(w).bits.uop.ldq_idx === spec_wakeups(w).bits.uop.ldq_idx
        )
      ...
      when(
        spec_wakeups(w).valid
      ) { // This sent a speculative wakeup, now we need to undo it
  ...
  for (i <- 0 until numStqEntries) {
    ...
    assert(
      !(IsKilledByBranch(io.core.brupdate, false.B, stq_uop(i)) && stq_valid(
        i
      ) && stq_committed(i)),
  ...
  for (w <- 0 until coreWidth) {
    val commit_store =
      io.core.commit.valids(w) && io.core.commit.uops(w).uses_stq
    val commit_load =
      io.core.commit.valids(w) && io.core.commit.uops(w).uses_ldq
    ...
    when(commit_store) {
      ...
      assert(
        ldq_valid(temp_ldq_head),
        "[lsu] trying to commit an un-allocated load entry."
      )
      assert(
        (ldq_executed(temp_ldq_head) || ldq_forward_std_val(
          temp_ldq_head
        )) && ldq_succeeded(temp_ldq_head),
    ...
    if (MEMTRACE_PRINTF) {
      when(commit_store || commit_load) {
        ...
        val addr = Mux(
          commit_store,
          stq_addr(temp_stq_commit_head).bits,
          ldq_addr(temp_ldq_head).bits
        )
        val stdata = Mux(commit_store, stq_data(temp_stq_commit_head).bits, 0.U)
        val wbdata = Mux(
          commit_store,
          stq_debug_wb_data(temp_stq_commit_head),
          ldq_debug_wb_data(temp_ldq_head)
        )
        printf(
          "MT %x %x %x %x %x %x %x\n",
          io.core.tsc_reg,
  ...
  when(hella_state === h_ready) {
    ...
    for (w <- 0 until lsuWidth) {
      when(
        (io.dmem.resp(w).valid && io.dmem.resp(w).bits.is_hella) ||
...
class ForwardingAgeLogic(n: Int) extends Module() {
  ...
  for (i <- 0 until n) {
    ...
    when(
      i.U >= io.youngest
    ) // currently the tail points PAST last store, so use >=
    {
      age_mask(i) := false.B
    }
  }
  ...
  matches := Cat(io.matches & age_mask.asUInt, io.matches)
...
/** when age1 is younger (later) than age2, return true, when age1 is older
  * (earlier than age2, return false
  *
  * IdxAgeYoungerThan(entry1, entry2) = entry1.dep_mask(entry2) first entry must
  * be real idx second entry must be idx pointer
  */
